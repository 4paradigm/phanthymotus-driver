...
class QianjiaoDevice:
    def __init__(self, cfg: dict):
        ...
        self.connection_topic = str(cfg.get("connection_topic", "/qianjiao2_pro/connection"))
        self.attitude_topic = str(cfg.get("attitude_topic", "/qianjiao2_pro/attitude"))
        self.depth_topic = str(cfg.get("depth_topic", "/qianjiao2_pro/depth"))
        self.temperature_topic = str(cfg.get("temperature_topic", "/qianjiao2_pro/temperature"))
        self.battery_topic = str(cfg.get("battery_topic", "/qianjiao2_pro/battery"))
        self.imu_topic = str(cfg.get("imu_topic", "/qianjiao2_pro/imu"))
        self.position_topic = str(cfg.get("position_topic", "/qianjiao2_pro/position"))
        ...
        self._ros_node = None
        self._ros_connection_pub = None
        self._ros_attitude_pub = None
        self._ros_depth_pub = None
        self._ros_temperature_pub = None
        self._ros_battery_pub = None
        self._ros_imu_pub = None
        self._ros_position_pub = None
    ...
    def start_ros_status(self):
        ...
        try:
            ...
            self._ros_connection_pub = self._ros_node.create_publisher(String, self.connection_topic, qos)
            self._ros_attitude_pub = self._ros_node.create_publisher(String, self.attitude_topic, qos)
            self._ros_depth_pub = self._ros_node.create_publisher(String, self.depth_topic, qos)
            self._ros_temperature_pub = self._ros_node.create_publisher(String, self.temperature_topic, qos)
            ...
            self._ros_position_pub = self._ros_node.create_publisher(String, self.position_topic, qos)
            ...
            while not self._stop.is_set():
                try:
                    ...
                    if parsed is not None:
                        ...
                        if self._ros_connection_pub is not None:
                            from std_msgs.msg import String
                            msg = String()
                            msg.data = json.dumps(self._connection_snapshot(), separators=(",", ":"))
                            self._ros_connection_pub.publish(msg)
                            msg.data = json.dumps(self._attitude_snapshot(parsed), separators=(",", ":"))
                            self._ros_attitude_pub.publish(msg)
                            msg.data = json.dumps({"depth": parsed.get("depth")}, separators=(",", ":"))
                            self._ros_depth_pub.publish(msg)
                            msg.data = json.dumps({"temperature": parsed.get("temperature")}, separators=(",", ":"))
                            self._ros_temperature_pub.publish(msg)
                            msg.data = json.dumps(self._battery_snapshot(parsed), separators=(",", ":"))
                            self._ros_battery_pub.publish(msg)
                            msg.data = json.dumps(self._imu_snapshot(parsed), separators=(",", ":"))
                            self._ros_imu_pub.publish(msg)
                            msg.data = json.dumps({key: parsed.get(key, 0) for key in ("lat", "lon")}, separators=(",", ":"))
                            self._ros_position_pub.publish(msg)
    ...
    @staticmethod
    def _attitude_snapshot(status: dict[str, Any]) -> dict[str, Any]:
        return {key: status.get(key) for key in ("roll", "pitch", "yaw")}
    ...
    def get_tools(self):
        sensor = lambda name, description, topic: {"name": name, "type": "sensor", "description": description, "topic_out": [{"topic": topic, "format": "data/json"}], "inputSchema": {"type": "object"}}
        return [
            sensor("rov_connection", "潜蛟连接状态和通信链路", self.connection_topic),
            sensor("rov_attitude", "潜蛟横滚、俯仰和航向", self.attitude_topic),
            sensor("rov_depth", "潜蛟实时深度", self.depth_topic),
            sensor("rov_temperature", "潜蛟实时温度", self.temperature_topic),
            sensor("rov_battery", "潜蛟电池状态", self.battery_topic),
            sensor("rov_imu", "潜蛟 IMU 陀螺仪", self.imu_topic),
            sensor("rov_position", "潜蛟位置", self.position_topic),
            {"name": "rov_camera", "type": "sensor", "description": "潜蛟实时视频流", "topic_out": [{"topic": self.camera_topic, "format": "image/jpeg"}], "inputSchema": {"type": "object"}},
    ...
    def dispatch(self, tool: str, args: dict) -> dict:
        ...
        status = self.status()
        if tool == "rov_connection": return self._connection_snapshot()
        if tool == "rov_attitude": return self._attitude_snapshot(status["rov"])
        if tool == "rov_depth": return {"depth": status["rov"].get("depth")}
        if tool == "rov_temperature": return {"temperature": status["rov"].get("temperature")}
        if tool == "rov_position": return {key: status["rov"].get(key, 0) for key in ("lat", "lon")}
        if tool == "rov_camera":
            return {"state": "available", "stream_url": self.video_url, "source_rtsp": self.camera_rtsp}
        if tool == "rov_battery":
            return self._battery_snapshot(self._rov_status)
        if tool == "rov_imu":
            return self._imu_snapshot(self._rov_status)
