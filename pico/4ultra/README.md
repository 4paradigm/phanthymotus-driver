...
- `PICO_PUBLIC_HOST` 或机器配置 `pico.public_host` 指定头显可达地址；未指定时通过本机路由选择地址，不发送探测数据。TLS与配对保存在 `/var/lib/motus/pico`。普通重建不换证书，IP改变也不静默覆盖信任身份。配对凭据加载与旧实例发现共用安全读取入口：以 O_NOFOLLOW 打开目录及文件，在同一个文件描述符上核验普通文件/0600，再有限读取 JSON；父目录要求 0700。链接、管道、错误权限或超大文件明确报错。
