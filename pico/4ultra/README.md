...
- `PICO_PUBLIC_HOST` 或机器配置 `pico.public_host` 指定头显可达地址；未指定时通过本机路由选择地址，不发送探测数据。TLS与配对保存在 `/var/lib/motus/pico`。普通重建不换证书，IP改变也不静默覆盖信任身份。配对凭据加载与旧实例发现共用安全读取入口：以 O_NOFOLLOW 打开目录及文件，在同一个文件描述符上核验普通文件/0600，再有限读取 JSON；父目录要求 0700。链接、管道、错误权限或超大文件明确报错。TLS 目录须由当前进程用户持有且权限为 0700，证书和私钥须为该用户持有的普通 0600 文件；OpenSSL 通过已核验且保持打开的文件描述符加载，避免检查后原路径被替换。设备配置也使用独占随机临时文件和原子替换保存。
- 配置默认空闲，无安装确认门槛。首次齿轮保存或项目启动建立监听；`info`可无ROS/TLS实例地解析端口。
- 管理 PIN 由用户首次在齿轮配置中设置，没有默认 PIN；只接受四位 ASCII 数字（可含前导零）。Driver 只保存加盐哈希，不通过 `info/config` 返回 PIN。新旧 PIN 文件都必须为普通文件且权限 0600，父目录必须为非符号链接目录且权限 0700；加载和保存均核验，不合规时报错，不静默改权限或使用不安全状态。读取、临时文件创建和原子替换均使用已打开并核验所有权/0700 的父目录 fd；文件也核验当前用户所有权，父目录随后被重命名或替换不会重定向读写。写入使用独占随机临时文件再原子替换，不覆盖可预测临时路径。配置声明 `format=password` 和 `x-sensitive`，由普通 Core 的既有敏感字段机制处理导出脱敏。普通 Core 的本地配置数据库仍保存该配置值，密码类型仅提供界面掩码与分享脱敏，不代表 Core 数据库加密。齿轮字段沿用 `scope=instance` 以适配现有表单，Driver 只保存一份 PIN。旧 `pairing_admin_password` 不迁移为 PIN。
