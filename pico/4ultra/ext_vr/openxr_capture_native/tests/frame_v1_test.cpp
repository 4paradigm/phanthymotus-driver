...
void TriggerMustBeNeutralBeforeRearm() {
  auto sample=Sample(false,0.0);
  sample.left_input.buttons[0]=0.6;
  auto held=BuildFrameV1(FrameState{},sample,ArmOnly());
  Check(held.state.rearm_required,"non-neutral trigger must block rearm");
  auto neutral=BuildFrameV1(held.state,Sample(false,0.0),ArmOnly());
  auto armed=BuildFrameV1(neutral.state,Sample(true,1.0),ArmOnly());
  Check(armed.frame.deadman,"neutral trigger then regrip enables");
  sample=Sample(true,1.0);sample.right_input.buttons[0]=0.42;
  auto motion=BuildFrameV1(armed.state,sample,ArmOnly());
  Check(motion.frame.deadman,"trigger motion must not release arms");
  CheckNear(motion.frame.right_input.buttons[0],0.42,"continuous trigger preserved");
}

void NamedControlsSerializeAvailabilityAndFalseStates() {
  auto sample = Sample(false, 0.0);
  motus::openxr_capture::Controls controls;
  controls.buttons["x"] = {.available=true, .pressed=false, .touched=true};
  controls.buttons["thumbstick"] = {.available=false};
  controls.axes["thumbstick"] = {.available=true, .value=std::array<double,2>{-0.4,0.8}};
  sample.left_input.controls = controls;
  const auto result = BuildFrameV1(FrameState{}, sample, ArmOnly());
  const auto json = SerializeFrameV1(result.frame);
  Check(json.find("\"x\":{\"available\":true,\"pressed\":false,\"touched\":true}") != std::string::npos,
        "named buttons preserve false pressed independently of touched");
  Check(json.find("\"thumbstick\":{\"available\":false}") != std::string::npos,
        "unavailable controls must not fabricate measurements");
  Check(json.find("\"axes\":{\"thumbstick\":{\"available\":true,\"value\":[") != std::string::npos,
        "named thumbstick axes must reach wire JSON");
}

}  // namespace

int main() {
  NamedControlsSerializeAvailabilityAndFalseStates();
  TriggerMustBeNeutralBeforeRearm();
