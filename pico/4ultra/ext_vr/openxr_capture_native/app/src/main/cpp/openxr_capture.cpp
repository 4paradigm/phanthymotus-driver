...
void Trace(const char* message) {
  __android_log_write(ANDROID_LOG_INFO, "MotusOpenXrCapture", message);
}
...
void OpenXrCapture::Initialize(android_app* app) {
  ...
  Trace("OpenXR init: loader");
  InitializeLoader(app);
  Trace("OpenXR init: instance");
  CreateInstance(app);
  Trace("OpenXR init: EGL");
  CreateEglContext();
  Trace("OpenXR init: session");
  CreateSession();
  Trace("OpenXR init: spaces");
  CreateSpaces();
  Trace("OpenXR init: actions");
  CreateActions();
  Trace("OpenXR init: swapchains");
  CreateSwapchains();
  Trace("OpenXR init: complete");
}
...
void OpenXrCapture::CreatePassthrough() {
  Trace("OpenXR passthrough: properties");
  XrSystemPassthroughPropertiesFB passthrough_properties{XR_TYPE_SYSTEM_PASSTHROUGH_PROPERTIES_FB};
  XrSystemProperties properties{XR_TYPE_SYSTEM_PROPERTIES};
  properties.next = &passthrough_properties;
  Check(xrGetSystemProperties(instance_, system_id_, &properties), "passthrough properties");
  if (!passthrough_properties.supportsPassthrough) {
    throw std::runtime_error("site-view passthrough is required for teleoperation");
  }
  ...
  const auto load = [&](const char* name, auto* function) {
    __android_log_print(ANDROID_LOG_INFO, "MotusOpenXrCapture", "OpenXR passthrough: load %s", name);
    Check(xrGetInstanceProcAddr(instance_, name,
        reinterpret_cast<PFN_xrVoidFunction*>(function)), name);
    if (*function == nullptr) throw std::runtime_error(name);
  };
  load("xrCreatePassthroughFB", &create_passthrough);
  load("xrCreatePassthroughLayerFB", &create_layer);
  load("xrDestroyPassthroughFB", &destroy_passthrough_);
  load("xrDestroyPassthroughLayerFB", &destroy_passthrough_layer_);
  XrPassthroughCreateInfoFB info{XR_TYPE_PASSTHROUGH_CREATE_INFO_FB};
  info.flags = XR_PASSTHROUGH_IS_RUNNING_AT_CREATION_BIT_FB;
  Trace("OpenXR passthrough: create feature");
  Check(create_passthrough(session_, &info, &passthrough_), "xrCreatePassthroughFB");
  XrPassthroughLayerCreateInfoFB layer{XR_TYPE_PASSTHROUGH_LAYER_CREATE_INFO_FB};
  layer.passthrough = passthrough_;
  layer.flags = XR_PASSTHROUGH_IS_RUNNING_AT_CREATION_BIT_FB;
  layer.purpose = XR_PASSTHROUGH_LAYER_PURPOSE_RECONSTRUCTION_FB;
  Trace("OpenXR passthrough: create layer");
  Check(create_layer(session_, &layer, &passthrough_layer_), "xrCreatePassthroughLayerFB");
  passthrough_ready_ = true;
  Trace("OpenXR passthrough: complete");
}
...
void OpenXrCapture::CreateActions() {
  ...
  create_action(
      ...
      "Squeeze value",
      &squeeze_action_);
  create_action(XR_ACTION_TYPE_POSE_INPUT,"aim_pose","Aim pose",&aim_action_);
  create_action(XR_ACTION_TYPE_FLOAT_INPUT, "trigger_value", "Trigger", &trigger_action_);
  create_action(
  ...
  create_action(XR_ACTION_TYPE_BOOLEAN_INPUT, "trigger_click", "Trigger click", &trigger_click_);
  create_action(XR_ACTION_TYPE_BOOLEAN_INPUT, "trigger_touch", "Trigger touch", &trigger_touch_);
  create_action(XR_ACTION_TYPE_BOOLEAN_INPUT, "grip_click", "Grip click", &grip_click_);
  create_action(XR_ACTION_TYPE_BOOLEAN_INPUT, "thumbstick_click", "Thumbstick click", &thumbstick_click_);
  create_action(XR_ACTION_TYPE_BOOLEAN_INPUT, "thumbstick_touch", "Thumbstick touch", &thumbstick_touch_);
  create_action(XR_ACTION_TYPE_BOOLEAN_INPUT, "primary_click", "Primary click", &primary_click_);
  create_action(XR_ACTION_TYPE_BOOLEAN_INPUT, "primary_touch", "Primary touch", &primary_touch_);
  create_action(XR_ACTION_TYPE_BOOLEAN_INPUT, "secondary_click", "Secondary click", &secondary_click_);
  create_action(XR_ACTION_TYPE_BOOLEAN_INPUT, "secondary_touch", "Secondary touch", &secondary_touch_);
  ...
  for (std::size_t index = 0; index < hand_paths_.size(); ++index) {
    ...
    bindings.push_back({aim_action_, binding_path("/input/aim/pose")});
    ...
    bindings.push_back({trigger_action_, binding_path("/input/trigger/value")});
    bindings.push_back({thumbstick_action_, binding_path("/input/thumbstick")});
    bindings.push_back({trigger_touch_, binding_path("/input/trigger/touch")});
    bindings.push_back({thumbstick_click_, binding_path("/input/thumbstick/click")});
    bindings.push_back({thumbstick_touch_, binding_path("/input/thumbstick/touch")});
    bindings.push_back({primary_click_, binding_path(index == 0 ? "/input/x/click" : "/input/a/click")});
    bindings.push_back({primary_touch_, binding_path(index == 0 ? "/input/x/touch" : "/input/a/touch")});
    bindings.push_back({secondary_click_, binding_path(index == 0 ? "/input/y/click" : "/input/b/click")});
    bindings.push_back({secondary_touch_, binding_path(index == 0 ? "/input/y/touch" : "/input/b/touch")});
    // PICO profiles define these click paths; Oculus Touch does not.
    if (CompiledRuntimeProfile().target == HeadsetTarget::kPico) {
      bindings.push_back({trigger_click_, binding_path("/input/trigger/click")});
      bindings.push_back({grip_click_, binding_path("/input/squeeze/click")});
    }
  }
  ...
  for (std::size_t index = 0; index < grip_spaces_.size(); ++index) {
    ...
    Check(xrCreateActionSpace(session_, &space_info, &grip_spaces_[index]),
          "xrCreateActionSpace");
    space_info.action=aim_action_;
    Check(xrCreateActionSpace(session_, &space_info, &aim_spaces_[index]),"xrCreateAimSpace");
  }
}
...
void OpenXrCapture::PollEvents() {
  ...
  for (;;) {
    ...
    if (event.type == XR_TYPE_EVENT_DATA_SESSION_STATE_CHANGED) {
      ...
      const auto* changed = reinterpret_cast<const XrEventDataPassthroughStateChangedFB*>(&event);
      if (changed->flags & (XR_PASSTHROUGH_STATE_CHANGED_REINIT_REQUIRED_BIT_FB |
...
bool OpenXrCapture::RenderFrame(FrameSample* sample) {
  ...
  view_visible_ = frame_state.shouldRender == XR_TRUE && !projection_views.empty();
  ...
  std::array<const XrCompositionLayerBaseHeader*, 2> layers{
      reinterpret_cast<const XrCompositionLayerBaseHeader*>(&passthrough_layer), layer};
  end_info.layerCount = view_visible_ ? 2 : 0;
  end_info.layers = view_visible_ ? layers.data() : nullptr;
...
void OpenXrCapture::SyncActions(XrTime predicted_display_time, FrameSample* sample) {
  ...
  const auto fill_hand = [&](std::size_t index, PoseSample* pose, ControllerSample* input) {
    ...
    *pose = LocatePose(
    ...
    Check(xrGetActionStateFloat(session_, &get_info, &trigger), "trigger state");
    ...
    *input = ControllerSample{
        .active = interaction_profile_allowed && pose->valid &&
            squeeze.isActive == XR_TRUE && trigger.isActive == XR_TRUE &&
            thumbstick.isActive == XR_TRUE,
        .xr_standard = true,
        ...
        .buttons = {std::clamp(static_cast<double>(trigger.currentState), 0.0, 1.0), squeeze_value},
    };
    Controls controls;
    const auto boolean_state = [&](XrAction action) -> std::optional<bool> {
      get_info.action = action;
      XrActionStateBoolean value{XR_TYPE_ACTION_STATE_BOOLEAN};
      Check(xrGetActionStateBoolean(session_, &get_info, &value), "button state");
      if (!interaction_profile_allowed || value.isActive != XR_TRUE) return std::nullopt;
      return value.currentState == XR_TRUE;
    };
    const auto button = [&](const char* name, XrAction click, XrAction touch) {
      auto& value = controls.buttons[name];
      if (click != XR_NULL_HANDLE) value.pressed = boolean_state(click);
      if (touch != XR_NULL_HANDLE) value.touched = boolean_state(touch);
      value.available = value.pressed.has_value() || value.touched.has_value();
    };
    button("trigger", trigger_click_, trigger_touch_);
    button("grip", grip_click_, XR_NULL_HANDLE);
    button("thumbstick", thumbstick_click_, thumbstick_touch_);
    button(index == 0 ? "x" : "a", primary_click_, primary_touch_);
    button(index == 0 ? "y" : "b", secondary_click_, secondary_touch_);
    if (interaction_profile_allowed && trigger.isActive == XR_TRUE) {
      controls.buttons["trigger"].available = true;
      controls.buttons["trigger"].value = std::clamp(static_cast<double>(trigger.currentState), 0.0, 1.0);
    }
    if (interaction_profile_allowed && squeeze.isActive == XR_TRUE) {
      controls.buttons["grip"].available = true;
      controls.buttons["grip"].value = squeeze_value;
    }
    auto& stick = controls.axes["thumbstick"];
    stick.available = interaction_profile_allowed && thumbstick.isActive == XR_TRUE;
    if (stick.available) stick.value = std::array<double, 2>{axis_x, axis_y};
    input->controls = std::move(controls);
  };
  fill_hand(0, &sample->left_controller, &sample->left_input);
  fill_hand(1, &sample->right_controller, &sample->right_input);
  std::array<PoseSample,2> aim;
  for(int i=0;i<2;++i)aim[i]=LocatePose(aim_spaces_[i],predicted_display_time,HasAllowedInteractionProfile(i));
  panel_.Sample(sample->head,aim,*sample);
  ...
  static std::int64_t last_trace_ns = -2'000'000'000LL;
  if (*sample->monotonic_ns - last_trace_ns >= 2'000'000'000LL) {
    __android_log_print(
        ANDROID_LOG_INFO,
        "MotusOpenXrCapture",
        "OpenXR sample: tracking=%d/%d/%d active=%d/%d trigger=%.2f/%.2f squeeze=%.2f/%.2f",
        sample->head.valid,
        sample->left_controller.valid,
        sample->right_controller.valid,
        sample->left_input.active,
        sample->right_input.active,
        sample->left_input.buttons[0],
        sample->right_input.buttons[0],
        sample->left_input.buttons[1],
        sample->right_input.buttons[1]);
    last_trace_ns = *sample->monotonic_ns;
  }
}
...
void OpenXrCapture::RenderProjection(
  ...
  for (std::size_t index = 0; index < swapchains_.size(); ++index) {
    ...
    XrPosef center = views.front().pose;
    if (views.size()==2) {
      center.position.x=(views[0].pose.position.x+views[1].pose.position.x)*.5f;
      center.position.y=(views[0].pose.position.y+views[1].pose.position.y)*.5f;
      center.position.z=(views[0].pose.position.z+views[1].pose.position.z)*.5f;
    }
    overlay_.Draw(visualization_,views[index],center,panel_);
...
void OpenXrCapture::Destroy() noexcept {
  passthrough_ready_ = false;
  if (egl_context_ != EGL_NO_CONTEXT) overlay_.Destroy();
  ...
  for(auto& space:aim_spaces_)destroy_space(&space);
