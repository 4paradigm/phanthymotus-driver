...
NormalizedController NormalizeController(const ControllerSample& sample) {
  ...
  result.wire.controls = sample.controls;
...
void AppendController(std::string& output, const WireController& controller) {
  ...
  if (controller.controls) {
    output.append(",\"controls\":{\"buttons\":{");
    bool first = true;
    // Names originate only from fixed OpenXR action bindings, never user text.
    for (const auto& [name, button] : controller.controls->buttons) {
      if (!first) output.push_back(','); first = false;
      output.append("\"" + name + "\":{\"available\":");
      output.append(button.available ? "true" : "false");
      if (button.available) {
        if (button.value) { output.append(",\"value\":"); AppendNumber(output, *button.value); }
        if (button.pressed) { output.append(",\"pressed\":"); output.append(*button.pressed ? "true" : "false"); }
        if (button.touched) { output.append(",\"touched\":"); output.append(*button.touched ? "true" : "false"); }
      }
      output.push_back('}');
    }
    output.append("},\"axes\":{"); first = true;
    for (const auto& [name, axis] : controller.controls->axes) {
      if (!first) output.push_back(','); first = false;
      output.append("\"" + name + "\":{\"available\":");
      output.append(axis.available ? "true" : "false");
      if (axis.available && axis.value) { output.append(",\"value\":"); AppendArray(output, *axis.value); }
      output.push_back('}');
    }
    output.append("}}");
  }
...
FrameResult BuildFrameV1(
  ...
  const bool triggers_neutral = left.wire.buttons.size() >= 2 && right.wire.buttons.size() >= 2 &&
      left.wire.buttons[0] <= 0.05 && right.wire.buttons[0] <= 0.05;
  const bool motion_inputs_neutral = triggers_neutral && (!configuration.base_twist ||
