...
from common.teleop_contract import validate_controls
...
FRAME_OPTIONAL_FIELDS = frozenset({"base_twist", "extensions"})
...
def _validate_controller(value: Any, name: str) -> dict:
    controller = _strict_object(value, name, {"axes", "buttons"}, set(value) - {"axes", "buttons"} if isinstance(value, dict) else set())
    ...
    if "controls" in controller:
        try:
            validate_controls(controller["controls"])
        except ValueError as exc:
            raise ProtocolError("invalid_controls", str(exc)) from exc
    return result
...
def _normalize_frame_v1(value: Any, *, expected_mode: str) -> dict:
    ...
    frame = _strict_object(
        ...
        set(value) - set(FRAME_REQUIRED_FIELDS) if isinstance(value, dict) else set(),
    )
    ...
    if "extensions" in frame and not isinstance(frame["extensions"], dict):
        raise ProtocolError("invalid_extensions", "extensions must be an object")
...
def bind_rtc_frame_v1(
    ...
    *,
    ...
    if isinstance(value, dict) and RTC_PRIVATE_FIELDS.intersection(value):
        raise ProtocolError("unknown_field", "RTC frame must not supply authority")
    wire = _strict_object(
        value,
        "RTC frame",
        set(RTC_FRAME_REQUIRED_FIELDS),
        set(value) - set(RTC_FRAME_REQUIRED_FIELDS) if isinstance(value, dict) else set(),
    )
