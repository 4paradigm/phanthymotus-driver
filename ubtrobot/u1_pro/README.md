...
- `expression`: face and gesture actions exposed by declared readable names such as `smile` or `blink`; these are fixed mappings from the supported U1 Pro motion contract. Head motions are kept in the separate `head` card.
