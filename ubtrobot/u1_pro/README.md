...
The U1 SDK does not expose a documented switch for stopping or disabling the vendor's internal Agent process itself. The driver disables the supported autonomous entry points at startup: new wakeups, post-wakeup dialog continuation, and visual decisions/following/idle behavior. It can also interrupt the current vendor playback/action through `tts.interrupt` or during startup. It cannot disable the vendor ROS services, system processes, safety/control loops, or an already explicitly requested motion through a system switch; those remain vendor-owned. `system_controls` returns the vendor readback state, which confirms configuration but does not claim that unrelated safety or low-level services are stopped.
