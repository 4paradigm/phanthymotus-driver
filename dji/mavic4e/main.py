#!/usr/bin/env python3
"""
dji/mavic4e/main.py — DJI Mavic 4E/4T 无人机设备 bundle 统一入口。
...
class Mavic4EDeviceBundle:
    ...
    def _model_tool(self) -> dict:
        return {
            "name": "model",
            "type": "resource",
            "description": "DJI Mavic 4E/4T aircraft metadata (cameras, gimbal range, specs)",
            "inputSchema": {"type": "object", "properties": {}},
        }

    def dispatch(self, tool_name: str, args: dict) -> dict | None:
        if tool_name == "model":
            info_path = Path(__file__).parent / "resource" / "mavic4e_info.json"
...
_bundle: Mavic4EDeviceBundle | None = None
            ...
            try:
                if method == "initialize":
                    ok({
                        ...
                        "capabilities": {"tools": {}},
                        "serverInfo": {"name": "dji-mavic4e-bundle", "version": "1.0.0"},
                    })
...
def main():
    ...
    _bundle = Mavic4EDeviceBundle(cfg, namespace, executor, bridge)
    _bundle.start_all()
    ...
    _start_registration(mcp_port, cfg.get("name", "DJI Mavic 4E/4T"), "driver")
