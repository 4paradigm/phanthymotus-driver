...
插件：
  ...
  WaypointPlugin         (actuator)  — 航点任务 V3（仅 Mavic，M300 配置中禁用）
...
class CameraPlugin:
    ...
    def get_tool(self) -> dict:
        return {
            "name": "camera",
            "type": "actuator",
            "description": "Matrice 300 RTK 挂载相机管理：拍照、录像、变焦、对焦、曝光、存储查询；可用项取决于实际挂载。",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "action": {
...
class GimbalPlugin:
    ...
    # Conservative generic limits. Actual M300 payload limits are enforced by PSDK.
    PITCH_RANGE = (-90, 30)
    YAW_RANGE = (-180, 180)
    ...
    def get_tool(self) -> dict:
        return {
            ...
            "type": "actuator",
            "description": (
                "Matrice 300 RTK 挂载云台控制：旋转 (pitch -90°~+30°, yaw -180°~+180°)、复位、模式切换。"
            ...
            "inputSchema": {
                ...
                "properties": {
                    ...
                    "pitch": {"type": "number", "description": "俯仰角 (度), -90~+30"},
                    "yaw": {"type": "number", "description": "偏航角 (度), -180~+180"},
