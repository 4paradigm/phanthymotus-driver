...
/*
 * PSDK Liveview for Matrice 300 RTK — FFmpeg H.264 decode + scale to 720x540 + JPEG.
...
int liveview_start(const char *camera, liveview_frame_cb_t cb) {
    ...
    /* M300 streams the active camera on payload port 1. Do not use M3T-only
     * sources or lens selectors: H20/H20T and third-party payloads expose
     * their supported source through PSDK's DEFAULT source. */
    (void)camera;
