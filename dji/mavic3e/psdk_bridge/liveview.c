...
static T_DjiReturnCode _h264_cb(E_DjiLiveViewCameraPosition pos,
...
int liveview_start(const char *camera, liveview_frame_cb_t cb) {
    ...
    /* camera: "wide" = NO_1, "zoom" = NO_1 (same physical camera, different lens) */
    T_DjiReturnCode rc = DjiLiveview_StartH264Stream(pos, _h264_cb);
    if (rc != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
...
int liveview_stop(void) {
    DjiLiveview_StopH264Stream(DJI_LIVEVIEW_CAMERA_POSITION_NO_1);
    s_frame_cb = NULL;
    return 0;
}
