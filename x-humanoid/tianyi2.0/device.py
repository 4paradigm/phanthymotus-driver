...
class HealthCheckPlugin:
    ...
    def get_tool(self) -> dict:
        return {
            "name": "health_check",
            "type": "actuator",
            "description": "天轶2.0 全身体检：汇总底盘、电机、手部、电源、急停、IMU、自检等所有子系统状态，给出故障原因和操作建议。应在以下场景调用：①用户说\"检查一下机器人\"\"体检\"\"状态怎么样\"\"有没有故障\"等类似话语时；②Agent Core 发现机器人无法正常移动或操作（可能被急停锁住，也可能出现电机/电源故障），需要排查根因时。急停激活时会提示用户拔掉急停按钮并按遥控器A键自检。",
            "inputSchema": {
                "type": "object",
                "properties": {
