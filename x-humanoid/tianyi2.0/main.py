...
class TianyiDeviceBundle:
    def __init__(self, cfg: dict, namespace: str, ros2: DualDomainROS2, slamtec_client):
        ...
        # Standalone sensor plugins (7 cards)
        ...
        if plugins_cfg.get("battery", {}).get("enabled", False):
            from device import BatteryStatePlugin
            self._plugins.append(BatteryStatePlugin(plugins_cfg["battery"], namespace, ros2))
            print("[bundle] BatteryStatePlugin loaded")
        ...
        if plugins_cfg.get("estop", {}).get("enabled", False):
            from device import EstopStatePlugin
            self._plugins.append(EstopStatePlugin(plugins_cfg["estop"], namespace, ros2))
            print("[bundle] EstopStatePlugin loaded")
