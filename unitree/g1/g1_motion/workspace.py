    ...
    def _safe_transition(self, start, end, check_budget=lambda: None):
        ...
        while pending:
            ...
            try:
                ...
                if (str(exc) not in ('torso_collision','arm_collision','workspace_limit')
                        and not str(exc).startswith('g1_body_collision:')) or depth>=getattr(self,'transition_refinement_depth',4):
                    raise
