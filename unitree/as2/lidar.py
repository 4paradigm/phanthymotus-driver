"""AS2/Livox point-cloud sensor, following the Go2 lidar_cloud contract."""
import struct
...
class _LidarNode:
    def __init__(self, topic, executor):
        ...
        try:
            ...
            self.node.get_logger().info("AS2 lidar subscribed to rt/utlidar/cloud_deskewed")
        except Exception as exc:
            self.sub = None
            self.node.get_logger().warning(f"AS2 lidar unavailable: {exc}")
...
class LidarPlugin:
    ...
    def get_tool(self):
        return {"name": "lidar_cloud", "type": "sensor", "multiInstance": False,
                "description": f"AS2/Livox PointCloud2 passthrough. Binary format [uint32 point_step][uint32 point_count][raw data], published to {self.topic}",
                "inputSchema": {"type": "object", "properties": {}},
                "topic_out": [{"topic": self.topic, "format": "sensor/pointcloud"}]}
    def start(self): pass
    def stop(self): pass
    def dispatch(self, action, args):
        if action in ("start", "info"): return {"state": "running", "topic_out": [{"topic": self.topic, "format": "sensor/pointcloud"}]}
