"""
ControlledSpatialPlugin — 人工控制建图与导航 actuator (R1-EDU + Mid360)。
...
9. nav_status → 查看当前导航状态（进度、障碍物、是否到达）
"""
...
class ControlledSpatialPlugin:
    """Controlled mapping & navigation for R1-EDU + Mid360."""
    ...
    def __init__(self, plugin_config: dict, namespace: str, executor):
        ...
        # Navigation state (enhanced for R1 — from ctrl_info and task_result)
        self._nav_state = {
            "is_arrived": False,
            "progress": {"completion_percentage": 0.0, "used_time": 0.0, "last_time": 0.0},
            "obstacle": {"state": False, "time": 0},
            "state_machine": "idle",
            "is_pause": False,
            "target_pose": None,
        }

        # Subscribe DDS for pose + navigation status updates
        self._dds_subs = []
        try:
            ...
            key_info_sub = ChannelSubscriber("rt/slam_key_info", String_)
            key_info_sub.Init(self._on_slam_key_info, 10)
            self._dds_subs.append(key_info_sub)
            print("[ControlledSpatial] subscribed rt/slam_key_info")
        except Exception as e:
            print(f"[ControlledSpatial] failed to subscribe DDS topics: {e}")
    ...
    def _tool_def(self) -> dict:
        return {
            ...
            "multiInstance": False,
            "description": (
                "Controlled mapping & navigation for R1-EDU+Mid360 — user manually drives the robot during mapping. "
                "Supports: start/stop mapping, tag places, list/delete maps, load map, navigate between tags, "
                "check navigation status with progress and obstacle info."
            ),
            "inputSchema": {
                "type": "object",
                "properties": {
                ...
                "x-action-params": {
                    ...
                    "navigate_to_tag": {"params": ["tag_name"], "description": "Navigate to a tagged place (max 30m)"},
                    "navigate_to_pose": {"params": ["x", "y", "yaw"], "description": "Navigate to coordinates (max 30m)"},
                    ...
                    "nav_status": {"params": [], "description": "Get current navigation status (progress, obstacles, arrival)"},
                },
            },
        }
    ...
    def _on_slam_info(self, msg) -> None:
        ...
        if msg_type in ("pos_info", "mapping_info"):
            ...
            with self._lock:
                self._nav_state["is_arrived"] = ctrl_data.get("is_arrived", False)
                self._nav_state["is_pause"] = ctrl_data.get("stateMachine", {}).get("isPause", False)
                self._nav_state["state_machine"] = ctrl_data.get("stateMachine", {}).get("state", "idle")
                progress = ctrl_data.get("progress", {})
                if progress:
                    self._nav_state["progress"] = {
                        "completion_percentage": progress.get("completion_percentage", 0.0),
                        "used_time": progress.get("used_time", 0.0),
                        "last_time": progress.get("last_time", 0.0),
                    }
                obs = ctrl_data.get("obsInfo", {})
                if obs:
                    self._nav_state["obstacle"] = {
                        "state": obs.get("state", False),
                        "time": obs.get("time", 0),
                    }
                target = ctrl_data.get("targetPose")
                if target:
                    self._nav_state["target_pose"] = target
    ...
    def _on_slam_key_info(self, msg) -> None:
        ...
        if msg_type == "task_result":
            ...
            with self._lock:
                self._nav_state["is_arrived"] = task_data.get("is_arrived", False)

    def _get_pose(self) -> dict | None:
        with self._lock:
            return dict(self._current_pose) if self._current_pose else None

    def _get_nav_state(self) -> dict:
        with self._lock:
            return dict(self._nav_state)
    ...
    def dispatch(self, action: str, args: dict) -> dict | None:
        if action == "info":
            ...
            q_z = math.sin(yaw / 2)
            q_w = math.cos(yaw / 2)
            # Reset nav state before starting new navigation
            with self._lock:
                self._nav_state["is_arrived"] = False
                self._nav_state["progress"] = {"completion_percentage": 0.0, "used_time": 0.0, "last_time": 0.0}
            ...
            nav = self._get_nav_state()
            pose = self._get_pose()
            return {
                "is_arrived": nav["is_arrived"],
                "progress": nav["progress"],
                "obstacle": nav["obstacle"],
                "state_machine": nav["state_machine"],
                "is_pause": nav["is_pause"],
                "target_pose": nav["target_pose"],
                "current_pose": pose,
