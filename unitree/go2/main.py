...
class Go2DeviceBundle:
    def __init__(self, cfg: dict, namespace: str, executor, rpc_proxy: RpcProxy, network_iface: str = "eth0"):
        ...
        if plugins_cfg.get("speaker", {}).get("enabled", False):
            ...
            self._plugins.append(SpeakerPlugin(plugins_cfg["speaker"], namespace, executor, rpc_proxy, network_iface=network_iface))
        ...
        if plugins_cfg.get("spatial", {}).get("enabled", False):
            ...
            self._plugins.append(SpatialPlugin(spatial_cfg, namespace, executor, rpc_proxy))
