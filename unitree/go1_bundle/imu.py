"""
imu.py — Go1 IMU 状态卡（四元数 wxyz / 角速度 / 加速度 / 欧拉角 / 温度）。

自包含：一张卡 = 一个文件。main.py 按 config.yaml 里的卡名自动 import 并 make_plugin()。
数据来源：共享只读 client 的 snapshot()["imu"]（来自 HighState.imu）。
...
def build(snap: dict) -> dict:
    """snapshot -> IMU dict。公共头带 timestamp/control_level/fresh，无新包不伪造。"""
    ...
    if imu is None:
        d["available"] = False
        return d
    d.update(imu)                     # quaternion_wxyz / gyroscope_rad_s / accelerometer_m_s2 / rpy_rad / temperature_c
    d["attitude_may_drift"] = True    # 加速运动时姿态漂移，提示上层
    return d
    ...
    def __init__(self, plugin_config, namespace, executor, client):
        ...
        if _HAS_ROS2 and executor is not None:
            try:
                ...
                self._node.get_logger().info(f"go1 imu → {self._topic} @ {HZ}Hz")
