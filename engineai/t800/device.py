...
class VisionPlugin:
    ...
    def _on_cloud(self, msg, source: str) -> None:
        # 渲染端（agent-core pointcloud.js）默认轴映射按 G1/Livox 约定：
        # 屏幕向上 = -z_data。slam 云位于 odom 标准坐标系（z 轴朝上），
        # 直接透传在监控页渲染会上下颠倒——这里对 z 分量取反后再发布，
        # 与平台 sensor/pointcloud 渲染契约对齐。raw 为传感器坐标系
        # （z 轴随头部俯仰倾斜），仅作调试源使用。
