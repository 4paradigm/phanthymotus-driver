...
class GesturePlugin:
    ...
    def _start_sequence(self, label: str, steps: list[dict], *, reset_after: bool) -> dict:
        ...
        def run() -> None:
            ...
            try:
                ...
                cancelled = self._cancel.is_set()
                if request_id is not None:
                    self._request_cancel(request_id, owner=owner)
                with self._lock:
                    self._status["state"] = "cancelled" if cancelled else "error"
                    self._status["error"] = "" if cancelled else str(exc)
            finally:
    ...
    def _stop(self, *, reset_after: bool) -> dict:
        with self._lock:
            ...
            result = dict(self._status)
        if request_id is not None:
            result["cancel_result"] = self._request_cancel(int(request_id), owner=owner)
        # 必须等 worker 退出后再释放锁，否则 worker 可能已通过取消检查、
        # 正准备调用 _dispatch_owned，与新拿到锁的动作竞态（迟到的手势
        # 请求会报 arm/head is busy 或与新动作并发）。这里只做有界等待；若
        # worker 尚未退出，不提前释放锁，motion_active 也会继续关闭整机运动门。
    ...
    def _request_cancel(self, request_id: int, *, owner: str | None = None) -> dict:
        dispatch_owner = owner or self._owner or "gesture"
        try:
            result = self._joint_plan._dispatch_owned(
                dispatch_owner, "cancel", {"request_id": int(request_id)}
            )
        except Exception as exc:
