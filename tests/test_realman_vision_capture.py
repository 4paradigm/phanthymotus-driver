"""RealMan capture port regression tests; real FFmpeg encoding, with a simulated ROS source."""
...
def load_module():
    ...
    spec = importlib.util.spec_from_file_location("realman_vision_capture_test", ROOT / "realman/rm75_6f_v/vision_capture.py")
...
def external_plugin(instances):
    ...
    tree = ast.parse((ROOT / "realman/rm75_6f_v/camera.py").read_text())
    cls = next(node for node in tree.body if isinstance(node, ast.ClassDef) and node.name == "ExtCameraPlugin")
    ns = {"TOPIC_FORMATS": {"rgb": "image/jpeg", "depth": "image/depth-zlib", "infrared": "image/jpeg"}}
...
class CaptureHarness(unittest.TestCase):
    def setUp(self):
        ...
        self.plugin = capture.VisionCapturePlugin({"output_dir": self.directory.name}, "go2", self.executor,
                                                  external_plugin({"color": external_status("/go2/ext_camera/color/rgb")}))
    ...
    def feed(self, data=b"\xff\xd8test\xff\xd9", age=0, fmt="jpeg", topic="/go2/ext_camera/color/rgb"):
    ...
    def publish(self, data, topic="/go2/ext_camera/color/rgb", fps=15):
...
class CaptureTest(CaptureHarness):
    def test_node_uses_initialized_executor_context(self):
        self.assertIs(self.plugin._node.context, self.executor.context)
        self.executor.add_node.assert_called_once_with(self.plugin._node)

    def test_instance_selection_is_exposed_in_action_and_config_schema(self):
        tool = self.plugin.get_tool()
        self.assertIn("external_instance_id", tool["inputSchema"]["properties"])
        self.assertIn("external_instance_id", tool["configSchema"]["properties"])
        for action in ("capture_photo", "record_video"):
            self.assertIn("external_instance_id", tool["inputSchema"]["x-action-params"][action]["params"])
        self.assertEqual(tool["configSchema"]["properties"]["camera"]["default"], "external")
        self.assertEqual(self.plugin.dispatch("config", {"camera": "front"})["code"], "INVALID_CAMERA")

    def test_missing_rgb_does_not_open_hardware_or_fall_back(self):
        self.plugin._external_camera = None
        result = self.plugin.dispatch("capture_photo", {})
        self.assertFalse(result["ok"])
        self.assertEqual(result["code"], "CAPTURE_FAILED")
        self.assertEqual(self.plugin.dispatch("list_cameras", {})["cameras"], [])

    def test_acp_acknowledgement_and_local_core_tls_contract(self):
        notify = capture.VisionCapturePlugin._notify_complete
        response = mock.MagicMock()
        response.__enter__.return_value.read.return_value = b'{"ok":true,"action_id":"test-action"}'
        with mock.patch.dict(capture.os.environ, {"AGENT_CORE_URL": "https://phanthy-motus:15678"}), \
             mock.patch.object(capture.urllib.request, "urlopen", return_value=response) as opened:
            notify(self.plugin, "test-action", "completed", {"ok": True})
        request = opened.call_args.args[0]
        self.assertEqual(request.full_url, "https://phanthy-motus:15678/api/acp/complete")
        self.assertEqual(json.loads(request.data)["action_id"], "test-action")
        response.__enter__.return_value.read.return_value = b'{"ok":true,"action_id":"other"}'
        with mock.patch.object(capture.urllib.request, "urlopen", return_value=response), \
             self.assertLogs("realman_vision_capture_test", level="WARNING") as logs:
            notify(self.plugin, "test-action", "completed", {"ok": True})
        self.assertIn("did not acknowledge", str(logs.output))
    ...
    def test_external_selection_excludes_depth_and_infrared_jpeg(self):
        ...
        self.assertEqual(len(self.plugin.dispatch("list_cameras", {})["cameras"]), 1)
    ...
    def test_driver_yaml_marketplace_lists_vision_capture(self):
        ...
        text = (ROOT / "realman" / "rm75_6f_v" / "driver.yaml").read_text()
    ...
    def test_cancel_waiting_for_first_frame_and_reject_overlap(self):
        with mock.patch.object(capture.shutil, "which", return_value="ffmpeg"):
            ...
            self.assertEqual(self.plugin.dispatch("config", {"external_instance_id": "other"})["code"], "RECORD_IN_PROGRESS")
            self.assertTrue(self.plugin.dispatch("config", {"camera": "external"})["ok"])
