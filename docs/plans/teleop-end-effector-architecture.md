...
本仓只修改天轶 Driver；用户确认设备空闲并开机后，补充机上只读预检、源码构建、禁网验证及明确授权的 Shadow 部署；本仓不改变 Agent Core、G1、servo、旧动作或 v1
...
9. 按 bot 意见补齐本地 bus 返回方向的逐帧解码、结构与序列化错误处理；坏反馈/关节结果不终止共享进程。保留旧 v1 命令由父执行器触发 HOLD 的规则。镜像显式关闭 ROS 颜色，不增加依赖。
10. 以独立 v2 契约补齐两卡 start 的描述符校验：严格版本/模式/整数维数/单位/分组、当前模型/标定/坐标系、声明速率与关节限位；Core 当前原样转发这些字段，不伪装成速率协商。EEF 保留末端位姿及 effector_ids，不套用 v1 的关节或力控字段要求。允许未知扩展字段，保留无描述符旧启动、v1 和逐帧实际门禁；新回归只运行离线，不切换现场服务。
...
- 后续 `388fe78` 已部署 Shadow（Live 关闭）。真实 ROS 预览链记录 45 帧 EEF 输入、44 个关联决策、190 帧反馈，ActuCore 收到 IK 显示；joint、厂商 cmd_pos 和 cmd_ctrl 计数均为零，Preview 释放成功。这一证据来自主任务的部署记录，不等于真机物理跟随或收臂验收。
- 2026-09-23 bot 边界修复：Mac Python 3.13 下 bus/输入/日志/两种构建入口 **65 passed**；既有 ARM64 镜像挂载候选源码、禁网只读验证同一 bus/输入/日志集合，Python 3.10 **57 passed**。其中 30 个新增用例运行真实子进程；在隔离 `388fe78` 原源码上，6 个 legacy 坏命令用例原本通过，而两个父端截断反馈用例使子进程退出，已区分实际 IPC 方向。新补丁未构建发布、未部署，也未操作机器人。
- 扩展 MotionGate、两卡生命周期、旧恢复与全仓 Docker COPY 回归为 **107 passed / 1 failed**；唯一失败是未修改的 Adam 测试互相导入被生产 COPY 检查识别。在隔离 `388fe78` 原源码重现完全相同失败，不修改无关 Adam 或全仓扫描规则来掩盖它。
- 2026-09-23 v2 描述符补齐：新增 **111 项**离线用例，真实调用 motion_control 与 arm 的 start/dispatch，验证坏声明拒绝后正确声明仍可启动、无 claim/命令输出、双份端口声明一致、扩展字段与数值类型策略，以及 EEF 独立契约。Mac Python 3.13 连同现有数值闭环、MotionGate、管理恢复、生命周期和 bus/bundle 回归 **240 passed**；既有 ARM64 镜像只读挂载候选源码，在禁网 Python 3.10 下新增专项 **111 passed**。本补丁未构建为新镜像、未部署，不改变现场验收结论。
