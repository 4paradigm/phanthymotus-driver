...
- 雷达可视化：使用 ROS 兼容的 `/grid_map_3d` 局部融合点云，通过持续发布的 `/ODOM` 转换到固定坐标并按 8 cm 体素累计，内部最多保留 500000 个固定体素；为避免大消息阻塞 Agent Core/Canvas，每次从完整地图均匀抽取最多 80000 点，以 3 Hz 输出 `sensor/pointcloud`。体素首次写入后不再覆盖，达到上限后保留已有区域，重新启动卡片会清空累计结果；没有里程计位姿时降级为最近 5 帧短窗口。
- 旧部署兼容：当外部配置仍只有旧值 `max_points: 50000` 且没有 `publish_max_points` 时，Driver 自动迁移为内部累计 500000、Canvas 发布 80000；显式配置新参数时仍尊重外部设置。`state.lidar` 会返回实际生效上限及 `legacy_config_migrated`。
...
已在 M20 Pro 真机确认 `/grid_map_3d` 输入和 Canvas 雷达输出均约 9–10 Hz；实测单帧包含 574–1381 个有效点、无零点，覆盖约 8×5 米的局部范围。真机还确认 `/SLAM_ODOM` 无发布者、`/LIO_ODOM` 无实际样本，而 `/ODOM` 持续约 10 Hz，因此固定坐标累计使用 `/ODOM`；累计效果仍需使用本提交镜像复测。真实状态机、速度方向、选配件存在性、充电及 Pro 导航仍需继续验真。首次联调前请确认系统版本为 V1.1.8、外接主机接入 `10.21.31.x` 或 `10.21.33.x` 网段，并确保没有与 `planner` 或 `charge_manager` 并发发布 `/NAV_CMD`。
