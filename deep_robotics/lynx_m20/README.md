...
- 雷达可视化：使用 ROS 兼容的 `/grid_map_3d` 局部融合点云，通过持续发布的 `/ODOM` 转换到固定坐标并按 8 cm 体素累计，以 3 Hz、最多 50000 点输出 Canvas `sensor/pointcloud` 数据；体素首次写入后不再覆盖，达到上限后保留已有区域，重新启动卡片会清空累计结果；没有里程计位姿时降级为最近 5 帧短窗口。
