...
- 雷达可视化：使用 ROS 兼容的 `/grid_map_3d` 局部融合点云，通过 `/SLAM_ODOM` 转换到固定 `map` 坐标并按 5 cm 体素累计，以 3 Hz、最多 50000 点输出 Canvas `sensor/pointcloud` 数据；没有 SLAM 位姿时降级为最近 5 帧短窗口。
...
已在 M20 Pro 真机确认 `/grid_map_3d` 输入和 Canvas 雷达输出均约 9–10 Hz，`/SLAM_ODOM` 约 10 Hz；实测单帧包含 574–1381 个有效点、无零点，覆盖约 8×5 米的局部范围。固定坐标体素累计仍需使用本提交镜像复测。真实状态机、速度方向、选配件存在性、充电及 Pro 导航仍需继续验真。首次联调前请确认系统版本为 V1.1.8、外接主机接入 `10.21.31.x` 或 `10.21.33.x` 网段，并确保没有与 `planner` 或 `charge_manager` 并发发布 `/NAV_CMD`。
