...
# ── AudioFilePlayerPlugin (actuator) ─────────────────────────────────────────
# 播放用户上传的本地 WAV 音频文件。文件通过 HTTP 端点 /api/audio/upload 上传到
# 容器内 audio_dir，返回 file_id；随后调用 play(file_id) 异步播放（返回 action_id），
# stop 可随时中断。播放走 publish_external_audio_playback_stream 通道。

class AudioFilePlayerPlugin:
    PREFIX = "audio_file_player"

    def __init__(self, plugin_config: dict, namespace: str, executor, media_ctrl):
        self._media_ctrl = media_ctrl
        self._namespace = namespace
        self._audio_dir = Path(plugin_config.get("audio_dir", "/work/audio"))
        self._audio_dir.mkdir(parents=True, exist_ok=True)
        self._max_bytes = int(plugin_config.get("max_file_bytes", 50 * 1024 * 1024))
        self._manifest_path = self._audio_dir / "manifest.json"
        self._lock = threading.Lock()
        self._stop_event = threading.Event()
        self._play_state = {"active": False, "action_id": None, "filename": None}
    ...
    def get_tool(self) -> dict:
        return {
            ...
            "multiInstance": False,
            "description": (
                "Play a user-uploaded local WAV audio file through the Bumi speaker. "
                "Upload files via HTTP endpoint /api/audio/upload (returns file_id), "
                "then call play with file_id (async, returns action_id). stop interrupts "
                "playback at any time. list shows uploaded files; delete removes one."
            ),
            "inputSchema": {
                "type": "object",
                "properties": {
                    ...
                    "file_id": {
                        "type": "string",
                        "description": "File id returned by upload or list",
                    },
                    "filename": {
                        "type": "string",
                    ...
                    "volume": {
                        ...
                        "minimum": 0,
                        "maximum": 200,
                        "description": "Optional volume override (0-200)",
                    },
                    "loop": {
                        "type": "boolean",
    ...
    # ── upload (shared by HTTP endpoint and upload action) ──
    def upload(self, filename: str, data: bytes) -> dict:
        import uuid
        if not data:
            return {"error": "empty file"}
        if len(data) > self._max_bytes:
            return {"error": f"file too large: {len(data)} bytes > {self._max_bytes}"}
        if not (data[:4] == b"RIFF" and data[8:12] == b"WAVE"):
            return {"error": "not a valid WAV file (missing RIFF/WAVE header)"}
        name = Path(filename or "audio.wav").name or "audio.wav"
        if not name.lower().endswith(".wav"):
            return {"error": "only .wav files are supported"}
        file_id = uuid.uuid4().hex[:12]
        target = self._audio_dir / f"{file_id}.wav"
        target.write_bytes(data)
        duration_s = self._wav_duration(data)
        with self._lock:
    ...
    def _do_upload(self, args: dict) -> dict:
        ...
        return self.upload(p.name, data)

    # ── play ──
    def _resolve_file(self, args: dict):
        ...
        if file_id:
            p = self._audio_dir / f"{file_id}.wav"
            if p.exists():
                return p
            p2 = self._audio_dir / str(file_id)
            if p2.exists():
                return p2
        if filename:
    ...
    def _do_play(self, args: dict) -> dict:
        import uuid
        path = self._resolve_file(args)
        if path is None:
            return {"error": "file not found. Upload first, or pass a valid file_id / filename / file_path."}
        if not path.exists():
            return {"error": f"file not found: {path}"}
        self._stop_internal()
        pcm = self._load_wav_pcm(path)
        if pcm is None or len(pcm) == 0:
            return {"error": f"failed to decode wav: {path}"}
        volume = args.get("volume")
        loop = bool(args.get("loop", False))
        action_id = uuid.uuid4().hex[:12]
        self._stop_event.clear()
        self._play_state.update({"active": True, "action_id": action_id, "filename": path.name})
        self._play_thread = threading.Thread(
            target=self._play_loop, args=(action_id, pcm, volume, loop),
            daemon=True, name="audio_file_player_play",
        )
        self._play_thread.start()
        return {"action_id": action_id, "state": "playing", "filename": path.name, "loop": loop}

    def _play_loop(self, action_id: str, pcm, volume, loop: bool) -> None:
        import numpy as _np
        from mediacontrol_py import AudioStream
        ...
        prev_cue = None
        try:
            # 前置：唤醒 + 开启外部音频播放开关 + 临时关提示音（set 接口间隔均 ≥500ms）
            self._media_ctrl.wakeup()
            time.sleep(0.5)
            self._media_ctrl.set_external_custom_audio_data_to_playback_enable(True)
            time.sleep(0.5)
            try:
                prev_cue = self._media_ctrl.get_audio_cue_enable()
                self._media_ctrl.set_audio_cue_enable(False)
                time.sleep(0.5)
            except Exception as e:
                print(f"[audio_file_player] mute cue failed: {e}", flush=True)
            if volume is not None:
                try:
                    self._media_ctrl.set_volume(int(volume))
                    time.sleep(0.5)
                except Exception as e:
                    print(f"[audio_file_player] set_volume failed: {e}", flush=True)
            self._media_ctrl.resume_audio_playback()

            frame_n = 16000 * 2 // 100  # 320 samples = 10ms @ 16kHz stereo
            while True:
                for off in range(0, len(pcm), frame_n):
                    if stop.is_set():
                        break
                    chunk = pcm[off:off + frame_n]
                    if len(chunk) < frame_n:
                        chunk = _np.pad(chunk, (0, frame_n - len(chunk)), mode='constant')
                    s = AudioStream()
                    s.channels = 2
                    s.sample_rate = 16000
                    s.format = 2
            ...
            print(f"[audio_file_player] play loop error: {e}", flush=True)
        finally:
            # 恢复提示音
            if prev_cue is not None:
                try:
                    time.sleep(0.5)
                    self._media_ctrl.set_audio_cue_enable(prev_cue)
                except Exception as e:
                    print(f"[audio_file_player] restore cue failed: {e}", flush=True)
            if self._play_state.get("action_id") == action_id:
                self._play_state.update({"active": False, "action_id": None, "filename": None})
    ...
    def _do_list(self) -> dict:
        ...
        return {"files": items, "audio_dir": str(self._audio_dir)}
    ...
    def _load_wav_pcm(self, path):
        ...
        try:
            ...
            print(f"[audio_file_player] wave open failed: {e}", flush=True)
            return None
        if n_frames == 0:
            return None
        # 字节 → float64（统一动态范围）
        if sw == 2:
            data = _np.frombuffer(raw, dtype='<i2').astype(_np.float64)
        elif sw == 1:
            data = (_np.frombuffer(raw, dtype='u1').astype(_np.float64) - 128.0) * 256.0
        elif sw == 3:
            b = _np.frombuffer(raw, dtype='u1').reshape(-1, 3)
            i32 = b[:, 0].astype(_np.int32) | (b[:, 1].astype(_np.int32) << 8) | (b[:, 2].astype(_np.int32) << 16)
            i32 = _np.where(i32 & 0x800000, i32 | ~0xFFFFFF, i32)
            data = i32.astype(_np.float64) / 256.0
        elif sw == 4:
            data = _np.frombuffer(raw, dtype='<i4').astype(_np.float64) / 65536.0
        else:
            print(f"[audio_file_player] unsupported sampwidth {sw}", flush=True)
            return None
        data = data.reshape(-1, n_ch)
        if rate != 16000:
            data = self._resample(data, rate, 16000)
        data = _np.clip(_np.round(data), -32768, 32767).astype(_np.int16)
        if data.shape[1] == 1:
            stereo = _np.repeat(data, 2, axis=1)
        else:
            stereo = data[:, :2]
    ...
    def _resample(data, src_rate: int, dst_rate: int):
        import numpy as _np
        n = data.shape[0]
        src_t = _np.arange(n) / float(src_rate)
        dst_n = int(round(n * float(dst_rate) / src_rate))
        dst_t = _np.arange(dst_n) / float(dst_rate)
        out = _np.zeros((dst_n, data.shape[1]), dtype=_np.float64)
        for c in range(data.shape[1]):
            out[:, c] = _np.interp(dst_t, src_t, data[:, c])
        return out
