...
until the current action reaches a terminal state. Terminal callbacks use bounded
retries and remain registered in the driver until Agent Core acknowledges
delivery, preventing a fast completion or a transient localhost/registration
race from silently losing the result.
