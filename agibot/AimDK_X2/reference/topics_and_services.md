...
| `/aima/hal/sensor/lidar_chest_front/lidar_pointcloud` | `lidar` | `sensor/pointcloud`; live unit has one `PointCloud2` publisher (`RELIABLE/TRANSIENT_LOCAL`), and the driver uses matching lidar QoS |
...
| `/aima/hal/sensor/rgbd_head_front/depth_image` | `camera_depth` | live unit has one `sensor_msgs/msg/Image` publisher (`RELIABLE/VOLATILE`); driver subscribes with the matching image QoS |
