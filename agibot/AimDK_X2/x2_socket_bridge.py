...
class TopicHandler:
    def __init__(self, topic: str, type_name: str, ctx: Context, executor: Any):
        ...
        self.node = Node(
            f"x2_bridge_{topic.strip('/').replace('/', '_')}",
            context=ctx,
        )
...
class Server:
    ...
    def client(self, conn: socket.socket) -> None:
        try:
            ...
            if raw_len is None:
                return
            metadata = json.loads(_read_exact(conn, struct.unpack("<I", raw_len)[0]))
            ...
            while not self.stop_event.is_set():
                ...
                if raw_len is None:
                    break
                payload = _read_exact(conn, struct.unpack("<I", raw_len)[0])
                if payload is None:
                    break
    ...
    def start(self) -> None:
        ...
        print(
            "[x2-socket-bridge] ready domain=42 "
