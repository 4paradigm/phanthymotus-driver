...
卡片的单次观察及搬运后的返回观察不设总时限，输入就绪、到位停稳和 RGB-D/VOP 同步可持续等待。
抓放阶段仍有 120 秒预算，抓放单段运动最多等待 45 秒；取消、设备故障、反馈异常和运动停滞检查保留。
Core 的 ACP 等待上限仍为 210 秒，与卡片观察等待独立；框架超时不代表卡片已经停止，不能据此重复下发。
需要结束观察等待时调用 `cancel`。本卡片不修改 Core 的超时行为。
...
抓放阶段有 120 秒预算，每段运动最多等待 45 秒；返回观察及可选拍照不设总时限。
框架 ACP 等待上限仍为 210 秒，框架超时不会取消卡片动作；不要据此重试，结束等待使用 `cancel`。
