"""Persistent JPEG photos and asynchronous MP4 recording from RealMan ROS images.
...
class VisionCapturePlugin:
    ...
    def __init__(self, plugin_config, namespace, executor, external_camera=None):
        ...
        if self._camera != "external":
            raise ValueError("RealMan vision_capture requires an external RGB camera")
        self._output_dir = Path(plugin_config.get(
            "output_dir", "/opt/phanthy-motus/data/vision_capture/realman")).expanduser()
    ...
    def get_tool(self):
        camera_property = {"type": "string", "enum": ["external"],
                           "description": "RealSense RGB 相机；复用已启动的 ext_camera 实例。"}
        instance_property = {"type": "string", "default": "",
                             "description": "RGB ext_camera 的 instance_id；仅有一路时可留空。"}
        return {
            "name": self.PREFIX, "type": "actuator", "multiInstance": False,
            "description": "Save a RealMan RGB JPEG photo or record a silent H.264 MP4 video to persistent storage.",
            "inputSchema": {
                "type": "object",
                "properties": {
                ...
                "x-action-params": {
                    "start": {"params": [], "description": "启用拍照和录像卡。"},
                    "capture_photo": {"params": ["camera", "external_instance_id"], "description": "保存所选相机的 RGB 照片为 JPG。"},
            ...
            "configSchema": {"type": "object", "properties": {
                "camera": {**camera_property, "default": "external"},
                "external_instance_id": instance_property,
            }},
        }
    ...
    def _resolve_source(self, args):
        ...
        if camera == "external":
            ...
            if len(candidates) != 1:
                raise ValueError("Multiple RGB external cameras are running; select external_instance_id before capture")
            source = candidates[0]
        else:
            raise ValueError("RealMan vision_capture requires an external RGB camera")
        topic = source["topic"]
    ...
    def _on_frame(self, topic, msg):
        ...
        # RealMan camera publishers stamp their frames. Reject
    ...
    def _notify_complete(self, action_id, status, result):
        ...
        try:
            ...
            with urllib.request.urlopen(request, timeout=3, context=ctx) as response:
    ...
    def _start_video_recording(self, args):
        ...
        with self._recording_lock:
            ...
            thread = threading.Thread(target=self._record_video_async, args=(active,),
                                      daemon=True, name="realman_vision_capture_record_video")
    ...
    def dispatch(self, action, args):
        if action == "config":
            with self._recording_lock:
                camera = args.get("camera", self._camera)
                instance_id = args.get("external_instance_id", self._external_instance_id)
                if camera != "external" or not isinstance(instance_id, str):
