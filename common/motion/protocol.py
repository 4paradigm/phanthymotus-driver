...
def sign(body, secret):
    return hmac.new(bytes.fromhex(secret), json.dumps(body, sort_keys=True, separators=(',', ':'),
...
def validate_descriptor(value, expected):
    ...
    require(type(value.get('dof')) is int and value['dof'] == expected['dof'])
    ...
    if mode == 'eef_pose':
        ...
        require(isinstance(names, list) and len(names) == expected['dof']
                and all(isinstance(name, str) and bool(name) for name in names))
        require(len(set(names)) == expected['dof'] and names == expected['joint_names'])
        limits = value.get('limits')
        require(isinstance(limits, dict))
        for name in ('lower', 'upper', 'max_velocity'):
            items = limits.get(name)
            require(isinstance(items, list) and len(items) == expected['dof'] and all(number(x) for x in items))
...
def validate(packet, lease, *, mode, now, model_version, calibration_version,
             frame, previous_seq=-1, previous_epoch=-1, dof=14):
    ...
    if body['schema'] != SCHEMA or body['mode'] != mode or type(body['dof']) is not int or body['dof'] != dof:
    ...
    values = vector(body['values'], dof, 'values')
    if mode == 'eef_pose':
        for offset in (3, 10):
...
def envelope(lease, *, seq, source_seq, mapping_epoch, generated_ns,
             valid_until_ns, mode, values, model_version, calibration_version, frame, dof=14):
    body = dict(schema=SCHEMA, boot_id=lease['boot_id'], session_id=lease['session_id'],
        ...
        dof=dof, values=list(values), model_version=model_version,
