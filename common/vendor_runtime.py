...
class DualDomainROS2:
    ...
    ``core_transport=\"socket\"`` is used by drivers whose robot-facing and
    Core-facing Fast DDS participants require different process-wide transport
    profiles.  In that mode the driver process owns only the robot participant;
    a driver-specific sidecar process owns Core DDS.
    """
