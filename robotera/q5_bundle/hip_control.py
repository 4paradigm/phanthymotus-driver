...
DESC = f"Q5 髋关节控制：set 设角度(°)，zero 归零。范围 {LOWER_DEG}° ~ {UPPER_DEG}°"
...
class Plugin:
        ...
        return {"name": CARD, "type": TYPE, "multiInstance": False, "description": DESC,
                ...
                "x-action-params": {
                    "set": {"params": ["position_deg"],
                            "description": f"设置髋关节角度（度），须在 {LOWER_DEG}°~{UPPER_DEG}° 范围内。"},
                    "zero": {"params": [], "description": "髋关节归零（回到 0°）。"},
                },
                "x-completion": {
                    "actions": ["set", "zero"],
                    "timeout": 30,
    ...
    def _allowed(self, target_rad):
        ...
        if status["lifecycle_state"] != "active" or not status["joint_state_fresh"]:
            return _failure("ROBOT_NOT_READY", "Q5 must be active with fresh /joint_states before hip control",
                            status=status)
        q5_ready, q5_status = q5_is_control_ready(self._client)
        if not q5_ready:
            return _failure("Q5_FSM_NOT_READY", "Q5 /xbot_state must be fresh and READY or ACTIVE before hip control",
                            status={**status, "q5_fsm": q5_status})
        if JOINT not in self._client.snapshot().get("joints", {}):
            return _failure("JOINT_MODEL_MISMATCH", "hip_joint absent from /joint_states", joint_name=JOINT)
    ...
    def _launch(self, target_rad, action_name, target_deg, action_id):
        ...
        with self._lock:
            if self._thread is not None and self._thread.is_alive():
                self._router.release(CARD)
                return _failure("MOTION_IN_PROGRESS", "A hip move is already active; call stop first")
