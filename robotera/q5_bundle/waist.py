"""Direct absolute waist-yaw position control for the Q5 model."""
...
WAIST_ACTIONS = {
    "waist_yaw": {
        "joint_name": "waist_yaw_joint", "title": "腰转：左右转动",
        "field": "waist_yaw_rad",
        "description": "范围[-1.57,1.57]rad；正负按坐标系，上半身转动、底盘不动。",
    },
}
DESC = "Q5 腰部控制：偏航（上半身左右转动，底盘不动）"
...
class Plugin:
    def __init__(self, plugin_config, namespace, executor, client):
        ...
        if min(self._max_step, self._rate) <= 0 or self._hold_repetitions < 1:
            raise ValueError("waist limits and rate must be positive")
    ...
    def _run(self, event, name, current, target, duration):
        ...
        try:
            ...
            # successful move sends the waist back to its start angle.
