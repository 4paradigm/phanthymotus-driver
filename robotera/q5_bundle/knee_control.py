...
DESC = f"Q5 膝关节控制：set 设角度(°)，zero 归零。范围 {LOWER_DEG}° ~ {UPPER_DEG}°"
...
class Plugin:
    ...
    def _allowed(self, target_rad):
        ...
        if status["lifecycle_state"] != "active" or not status["joint_state_fresh"]:
            return _failure("ROBOT_NOT_READY", "Q5 must be active with fresh /joint_states before knee control",
                            status=status)
        q5_ready, q5_status = q5_is_control_ready(self._client)
        if not q5_ready:
            return _failure("Q5_FSM_NOT_READY", "Q5 /xbot_state must be fresh and READY or ACTIVE before knee control",
                            status={**status, "q5_fsm": q5_status})
        if JOINT not in self._client.snapshot().get("joints", {}):
            return _failure("JOINT_MODEL_MISMATCH", "knee_joint absent from /joint_states", joint_name=JOINT)
